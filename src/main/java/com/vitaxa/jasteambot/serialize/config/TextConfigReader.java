...
public final class TextConfigReader {
    ...
    private final boolean ro; // read only
    private String skipped; // for whitespace and comments
    private int ch = -1;
    ...
    private BlockConfigEntry readBlock(int cc) throws IOException {
        Map<String, ConfigEntry<?>> map = MapHelper.newLinkedHashMapWithExpectedSize(16);
    ...
    private ConfigEntry<?> readEntry(int cc) throws IOException {
        ...
        if (ch == '-' || ch >= '0' && ch <= '9') {
            ...
            try {
                return readInteger(token, cc);
            } catch (NumberFormatException e) {
                return readLong(token, cc);
            }
        }
    ...
    private ConfigEntry<Integer> readInteger(String token, int cc) throws IOException {
        return new IntegerConfigEntry(Integer.parseInt(token), ro, cc);
    }

    private ConfigEntry<Long> readLong(String token, int cc) throws IOException {
        return new LongConfigEntry(Long.parseLong(token), ro, cc);
    }
